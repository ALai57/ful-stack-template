(ns cljs-spa.router
  (:require [goog.object :as gobj]
            ...
            ;;["router5-plugin-browser" :as router5-browser-plugin]
            ))
...
(comment
  (in-ns 'cljs-spa.router)
  (require '[cljs-spa.routes :as routes])
  (require 'router5-plugin-browser)
  (require '[cljs-spa.state :refer [!state]])
  ...
  (def router
    (router5/createRouter (clj->js routes/the-routes)))

  (.usePlugin ^js router
              ((.-default router5-plugin-browser) #js {:useHash true}))
  (.useMiddleware ^js router
                  (fn [router]
                    (fn [to-state from-state]
                      (middleware* name->route to-state from-state))))
